...
class ProgressBar(Gtk.Scale):
    """Escala de la Barra de progreso para mostrar estado de reproduccion."""
...
class ToolbarBalanceConfig(Gtk.Table):
    ...
    en Video. (Utilizado por JAMediaReproductor). """
    ...
    def __init__(self):
        ...
        self.brillo = ToolbarcontrolValores("Brillo", -1.0, 1.0)
        self.contraste = ToolbarcontrolValores("Contraste", 0.0, 2.0)
        self.saturacion = ToolbarcontrolValores("Saturación", 0.0, 2.0)
        self.hue = ToolbarcontrolValores("Hue", -1.0, 1.0)
        self.gamma = ToolbarcontrolValores("Gamma", 0.01,10)
        ...
        self.attach(self.gamma, 2, 3, 1, 2)
    ...
    def emit_senial(self, widget, valor, tipo):
        """Emite valor, que representa un valor float y
            un valor tipo para:
            brillo - contraste - saturacion - hue - gamma"""
            
        self.emit('valor', valor, tipo)
        
    def set_balanace(self, brillo = None, contraste = None,
...
class ToolbarcontrolValores(Gtk.Toolbar):
    ...
    def user_set_value(self, widget= None, valor= None):
        ...
        progreso cuando el usuario la desplaza y
        emite una señal con el valor modificado
        segun la escala de valores pasadas en min y max."""
        
        total = self.max - self.min
        valor = valor * total / 100
        if self.min < 0.0: valor -= 1.0
        self.emit('valor', valor)
        if self.min < 0.0: valor += 1.0
        valor = int(valor * 100 / total)
        self.frame.set_label("%s: %s%s" % (self.titulo, valor, "%"))
